...
## Milestone 5 第二階段（第二輪）— 換到論文宣稱有用的區間、敏感度、消融、修 oracle
**狀態**: PASS
**執行時間**: 2026-09-05 13:5x → 1x:xx
**動機**: 第一輪在 Mooncake／端到端上量，那裡 oracle headroom 只有 8.49%——
**方法在它自己宣稱沒用的區間被檢驗**。使用者指出應改用論文 §6.8 那個
「重用率高、長 prefill、headroom 34.6%」的設定。這一輪把三件事一起做完：
(1) 搬到那個設定、(2) 敏感度與消融、(3) 修掉第一輪發現的 oracle 缺陷。

### 這一輪改了什麼程式

| 改動 | 為什麼 |
|---|---|
| `m4_oracle.run_oracle` 新增 `dest="marginal"`，`dest="best"` 改取三條規則較佳者 | 第一輪發現「多給 512 GiB SSD 反而慢 3.3%」。原因是 `cost-aware` 拿整條尾巴當丟棄的邊際成本，而尾巴是該請求所有缺塊共用的——重複計價。**`sim_version` 因此由 `b0d39dd4` 變為 `fbea1a98`**，既有 M4 的 CSV 全部帶舊戳記 |
| `m4_invariants.check_capacity_monotone` | 把上面那條變成自動檢查：多給一階容量，oracle 的 `total_ms` 不得上升。`m4_sweep.py --axis ssd` 每條 trace 掃完會跑一次 |
| `m5_predictor` 新增兩個長上下文工作負載 | `longctx-zipf` 重現論文 §6.8 的 `surface:L131072:rq10`（重用 89.06%、壓力 6.41×，實測完全對上）；`longctx-session` 是多輪會談形態，兩者都讓**位置 32K–128K（κ 37–60）第一次有正樣本**——Mooncake 上 8K 以上一個都沒有 |
| `--reuse-features`（只重算標籤） | 敏感度掃描要跑十幾組 (W, 取樣率, seed)，而特徵不受這三者影響。重放一次兩分鐘、只算標籤三十秒 |
| `--label-mode uncensored` | 第一輪的發現 1：設限的迴歸目標把「W 之外」的 block 壓成同分，而策略吃的是排序 |
| `--feature-groups` / `--cross-test` | (C) 區塊的簡化版消融、以及 §5.2 承諾要報的跨工作負載泛化 |
| `test_pred_*.npz` 存測試段預測 | 門檻掃描與可靠度圖從這裡畫，不必重訓 |
| `notebooks/m5_figures.py` | 四張圖，每張都是**表格表達不了的形狀**（聯合分布／連續曲線／可靠度圖） |

### 主結果一：換到論文宣稱有用的區間，線上策略由拿 10% 變成拿 82%

同一份程式、同一個預測器設計、同一組成本常數，**只換工作負載**（評估段都是模型沒看過的後 30%）：

| 工作負載 | 計時 | oracle headroom | 線上策略 vs 最佳 baseline | 拿到 headroom |
|---|---|---|---|---|
| **長上下文 131K（論文 §6.8 的 34.6% 設定）** | prefill | 18.81% | **+15.40%** | **81.85%** |
| 同上 | 端到端 | 16.09% | +13.17% | 81.85% |
| Mooncake toolagent | 端到端 | 11.50% | +0.76% | 6.62% |

（最佳 baseline 分別是 `tier_fs`、`tier_fs`、`cpu_lru`。）
第一輪「只拿到 10%」不是方法的上限，是**工作負載的性質**：
Mooncake 中位請求 6.3K、重用集中在 8K 以下、decode 佔一半以上時間。

### 主結果二：那 82% 全部來自「預測的逐出順序」，成本模型單獨值 0%

加一個控制組——**逐出順序退回 LRU，目的地規則不變**（式 (9) 的門檻、成本模型照舊）：

| 設定 | LRU 排序 + 成本模型 | 預測排序 + 成本模型 | oracle headroom |
|---|---|---|---|
| 長上下文 131K（論文設定） | **+0.00%** | **+15.40%** | 18.81% |
| lc32ks（預測沒訊號） | −3.49% | **−458%** | 13.93% |
| Mooncake toolagent | （cascade 目的地：−7.79%） | +0.76% | 11.50% |

**第一列的 +0.00% 是逐位元等於 `tier_fs`**：$\hat p$ 高於門檻時成本模型永遠選「往下推」，
於是退化成 `tier_fs`。所以在長上下文區間，**是排序在賺，不是門檻**。
而 Mooncake 上相反（把目的地換成 cascade 就掉到 −7.79%，門檻才是賺的那一半）。
**兩個元件各自在不同區間才有價值，論文不能只講其中一個。**

### 主結果三 🔴：預測沒有訊息時，排序式策略比 LRU 差 2.7–5.6 倍

`lc128ks`（131K × 4 並行）的測試段上 oracle 與最佳 baseline **完全相同**（headroom 0.00%），
而線上策略慘敗：

| 策略 | 總時間 | GPU 命中 | CPU | SSD | 重算 |
|---|---|---|---|---|---|
| `tier_fs` = oracle | 1,649,587 ms | 49.0% | 37.3% | 1.1% | 12.6% |
| `cpu_lru` | 1,773,974 ms | 49.0% | 37.3% | 0% | 13.7% |
| **`tiara_sym_l2`** | **4,476,442 ms** | **14.1%** | 32.4% | **40.9%** | 12.6% |

**重算率完全一樣（12.6% = 強制未命中），差別全在「從哪一階拿回來」。**
機制：該負載上 $\hat p$ 中位數 0.768 且 **100% 的 block 都 > 0.5**，
預測值近乎常數 → $\hat\tau$ 近乎常數 → **逐出順序等於隨機** → GPU 命中率由 49% 掉到 14%。
`lc32ks` 上同一機制造成 5.6 倍（13.5M vs 2.42M ms），而把排序換回 LRU 只剩 −3.49%。

**這是論文必須寫的限制**：預測失效時，學習式策略**不會退化成「跟隨機一樣」，
而是遠差於 LRU**——LRU 的 recency 排序本身是一個很強的先驗，排序式策略在分數打平時把它丟掉了。
**工程上的直接後果：預測器需要一個「我不知道」的退路**（分數離散度低於門檻就回退 LRU）。

### 主結果四 🔴：論文那個 34.6% 的設定只有 120 個獨立事件

| 工作負載 | 請求（獨立事件） | 存取 | 每事件重複 | 訓練後的 AUC |
|---|---|---|---|---|
| lc128kz（論文設定） | **120** | 1,002,600 | 8,355 個 block | **0.500** |
| lc128ks（會談式 40 會談） | 278 | 2,322,690 | 8,355 | 0.561 |
| lc32ks（32K×16 並行×400 會談） | 2,782 | 5,808,816 | 2,088 | 0.595 |
| Mooncake toolagent | 23,608 | 12,694,731 | 538 | **0.9995** |

一份 131K 的文件是**一個事件**，它的 8,192 個 block 特徵與標籤幾乎相同。
「985,483 個樣本」實際只帶 120 個事件的資訊（訓練集 84 個）。
**AUC 0.5 不是方法學不動，是資料裡沒有東西可學**——但那個模型的 $\hat\tau$ 仍有弱排序
（Spearman 0.13），而**那點弱排序就足以拿到 82% 的 headroom**。

**論文層面的後果**：§6.8 那條 headroom 曲線是拿 120 個請求的合成 trace 掃出來的。
它可以支持「oracle 有多少空間」，**但不能支持任何關於學習式方法的主張**。
要在長上下文區間宣稱學習有效，需要**獨立事件數量級更大**的工作負載。

### 敏感度：seed 與取樣率不影響結論，視窗長度影響很大

| 變動 | AUC | Spearman | μs/決策 |
|---|---|---|---|
| seed 1234 / 7 / 99 | 0.9995 / 0.9995 / 0.9996 | 0.233 / 0.235 / 0.243 | 0.872 / 0.872 / 0.786 |
| 取樣率 0.05 / 0.25 / 1.0 | 0.9995 / 0.9995 / 0.9996 | 0.233 / 0.233 / 0.243 | 0.867 / 0.872 / 0.786 |
| 視窗 W = 1× / 4× / 35× | 0.9995 / 0.9946 / **0.8960** | 0.233 / 0.246 / **0.701** | 0.872 / 17.96 / 145.8 |

**seed 與取樣率是雜訊（三位小數內），視窗長度是一階效應。**
W 拉長 40 倍，AUC 由 0.9995 掉到 0.896 而排序品質由 0.23 升到 0.70——
兩個指標**往相反方向動**，再次說明它們量的不是同一件事。

### 特徵族消融：LRB 的 deltas/EDC 在這裡沒有貢獻

同一設定內比較（toolagent、W=1×、取樣 0.25、葉 63）：
...
| history+deltas+EDC（**沒有 static**） | 0.9995 | 0.122 | 824.23 ms |
| 全部 | 0.9995 | 0.233 | 829.00 ms |

**只用 static 特徵比用全部還好**（成本低 30%）。長上下文設定上同樣是 static 最好
（AUC 0.6312 vs 全部的 0.5000）。
§5.2 的特徵設計整個建立在 LRB 的 deltas+EDC 上，而**KV block 的重用是請求結構驅動的，
不是 CDN 那種物件熱度驅動的**——位置與請求長度才是訊號。

### 跨工作負載泛化：排序會轉移，校準不會

| 訓練於 | 測試於 | AUC | ECE | 成本加權錯誤 |
|---|---|---|---|---|
| toolagent | toolagent（同域） | 0.9995 | 0.0003 | 829 ms |
| conversation | **toolagent** | 0.9777 | **0.2874** | **3,385,691 ms** |
| conversation | conversation（同域） | 0.9988 | 0.0006 | 350 ms |
| toolagent | **conversation** | 0.9941 | 0.0006 | 5,889 ms |

**AUC 幾乎不掉（0.9995 → 0.9777）而 ECE 惡化 1,000 倍、成本惡化 4,000 倍。**
因為門檻施加在**校準後**的機率上，校準一崩，決策就全錯。

**這給了 §5.2 第 (ii)、(iii) 層一個具體的答案**：跨工作負載時**不必重訓 GBDT，
只要重擬 isotonic 校準**（幾百毫秒、只需要最近一小段樣本），
比 KVP 那種以「日」計的重訓便宜好幾個數量級。這是可以寫進論文的正面結果。

### 🔴 新發現的失效模式：預測沒有訊息時，排序式策略比 LRU 差 2.7 倍

`lc128ks`（131K 文件、4 個並行會談）的測試段上，oracle 與最佳 baseline **完全相同**
（headroom 0.00%）——這個設定沒有任何可爭取的空間。而線上策略在這裡**慘敗**：

| 策略 | 總時間 | GPU 命中 | CPU | SSD | 重算 |
|---|---|---|---|---|---|
| `tier_fs` / oracle | 1,649,587 ms | 49.0% / 67.3% | 37.3% / 20.1% | 1.1% / 0% | 12.6% |
| `cpu_lru` | 1,773,974 ms | 49.0% | 37.3% | 0% | 13.7% |
| **`tiara_sym_l2`** | **4,476,442 ms** | **14.1%** | 32.4% | **40.9%** | 12.6% |

**重算率完全一樣（12.6%，就是強制未命中），差別全在「從哪一階拿回來」**：
tiara 把 40.9% 的存取推到 SSD（10.245 ms/block），而 baseline 從 GPU（0 ms）拿。

機制：這個工作負載上 $\hat p$ 的中位數是 0.768 且 **100% 的 block 都 > 0.5**，
預測值幾乎是常數 → $\hat\tau$ 幾乎是常數 → **逐出順序等於隨機**。
於是 GPU 命中率由 49% 掉到 14%。

**這條值得寫進論文的限制**：一個「預測沒有訊息」的學習式策略**不會退化成隨機逐出的水準，
而是遠差於 LRU**——因為 LRU 的 recency 排序本身就是一個很強的先驗，
而排序式策略在分數打平時把那個先驗**丟掉了**。
實作上的直接後果：**預測器必須有一個「我不知道」的退路**（例如分數的離散度低於門檻時
回退到 LRU），否則在預測失效的工作負載上會造成 2.7 倍的損失。
這也解釋了為什麼 §5.2 只報「準確度」不夠——準確度不會告訴你分數有沒有離散度。

### 磁碟清理（要記一筆，因為它動到可追溯性）

`/ssd7` 用到 96%，本輪產生的特徵矩陣（每個 Mooncake 的 `X.npy` 約 2.2 GiB）
累積到 18 GiB。清掉 17 個**不再被 `features_index.json` 指到**的孤兒目錄，回收 14.6 GiB。

**代價**：第一輪 RUNLOG 引用的特徵 run_id（`20260905-131229-m5p-feat-*` 等）
其目錄已不存在。訓練 run（模型、校準表、`train_meta.json`）**都還在**，
被刪的只有特徵矩陣本身，且**完全可重現**：

```bash
python code/m5_predictor.py features --trace toolagent --model qwen-awq \
    --device nvme --window-mult 1.0        # seed 固定，輸出逐位元相同
```

### 失敗與異常（三個，全部有修）

**1. 校準集可能只有單一類別，而 isotonic 會把整個輸出壓成常數。**
合成的會談式負載上第一次跑就踩到：訓練期最後 15%（校準集）**全是正樣本**，
於是 isotonic 把所有輸入映到 $\hat p = 1$，`AUC 恰為 0.5000`、`Spearman = nan`、
`ECE = 0.3334`。三個指標都「有數字」，但模型的輸出根本沒被用到。
Mooncake 上不會發生（資料大而混雜），合成負載上很容易。
**修法**：偵測到單一類別就改用**訓練期內等距抽樣**當校準集（仍全部早於測試期，
不洩漏未來），並在 `predictor_metrics.csv` 記 `calib_split` 與 `calib_positive_rate`；
整個訓練期都單一類別則中止。

**2. `write_csv` 的 schema 守門把「欄位順序不同」誤判成「schema 改變」。**
兩條路徑（完整重放 vs 只重算標籤）組 dict 的次序不同，欄位集合其實一樣。
**修法**：集合相同就照既有檔頭重排；只有集合真的變了才中止。
另外把 `samples.csv` 的欄位順序釘死成 `SAMPLE_FIELDS`。

**3. 合成工作負載讓 `samples.csv` 的欄位集合改變。**
Mooncake 沒有 `doc_tokens`／`n_docs`／`alpha` 這些欄位。
**修法**：`WORKLOAD_FIELDS` 固定，缺的填空字串——所有工作負載共用同一組欄位。

### 文獻檢索（`deepsearch`）

| 找到的東西 | 對本文的影響 |
|---|---|
| **Zhao, H., 《Instance weighting versus threshold adjusting for cost-sensitive classification》, KAIS 15:321–334 (2008), doi 10.1007/s10115-007-0079-1** | 正面比較 §5.3 的兩個選項。結論：對 naive Bayes／LR／NN 兩者相近，**對決策樹、決策表、規則學習器差很多**。Tiara 用 GBDT，正是樹族。§5.3 目前把「加權會贏」寫成推導出來的預期，但這個問題**在文獻裡已被實證比較過**且沒有一致的贏家 |
| LRB 的 **good decision ratio** | 最接近本文 B5 的既有決策品質指標。LRB 把殘差歸因於「模型的預測誤差」而未再分解；本文把它拆成分類 vs 排序 |
| **LHD**（NSDI'18，hit density） | 「以經濟量排序而非以分類排序」的先例，**不在 `refs.bib` 裡**，B5 的建議應該引它 |
| CacheWise（arXiv 2606.16824） | 2026 的 coding-agent KV 管理：prefix-aware 排程 + reuse-aware 逐出，但用**工具呼叫中繼資料**做輕量預測、**沒有**重算 vs 傳輸的成本模型。相關工作，也可能是長上下文 trace 的來源 |
| MTDS（doi 10.1007/s40747-025-02200-4） | 多階 KV 儲存，明文「依**預測的未來命中機率**調整儲存優先序」。**目前找到最接近本文核心的既有工作，必須讀**（付費牆，本次沒讀到） |

⚠️ 以上皆為**摘要層級**資訊，未讀全文。依 `citation-audit` 的規矩，寫進論文前要逐篇確認。

### 產出的圖（`notebooks/m5_figures.py`，四張）

專案預設是**留表刪圖**，所以只畫表格表達不了的形狀：

| 圖 | 為什麼表格不行 |
|---|---|
| `m5_ordering` | 預測 vs 真值的**聯合分布**——要看到正樣本被壓成一條水平帶，才知道問題出在設限的目標 |
| `m5_threshold` | 成本對門檻的**連續曲線**與最小值位置；表格只能給端點 |
| `m5_calibration` | 可靠度圖。§B.6 明文要求：「缺少此圖，該缺陷等同未被處理」 |
| `m5_horizon` | 正樣本率／AUC／Spearman 對 $W$ 的趨勢與**交叉**（AUC 下降而 Spearman 上升） |

### 下一步（依價值排序）

1. **把「不確定就回退 LRU」實作出來**（主結果三）。判準可以直接用分數的離散度
   或校準後 $\hat p$ 的變異數。這是目前**唯一**會讓方法在所有測過的工作負載上都不輸的改動。
2. **重跑所有帶舊 `sim_version`（`b0d39dd4`／`0e7b989d`）的 M4 CSV**。
   oracle 修好之後 headroom 由 8.4% 升到 12.3%，`budget_sweep`、`headroom_surface`、
   `by_length`、`semantics_ablation` 全部要重算才能與新值並列。
3. **造一條獨立事件夠多的長上下文工作負載**（主結果四）。
   現有的合成器都是「一份文件 = 一個事件」，128K 下事件數必然很少。
   可能的方向：多文件拼接（RAG 形態）、或把 CacheWise 的 agent trace 轉進來。
4. **特徵改以 static 為主**（消融結果），並補上 §5.2 承諾但此處做不到的兩族
   （pooled KV、attn_mass）——那需要真的跑引擎。
5. §5.3 的 (L2) 排序損失：主結果二說排序是賺錢的那一半，而 (L2) 正是排序損失。

---

## M5 第二階段（第三輪）— 精度階、完整訓練評估、正式報告
**狀態**: PASS
**執行時間**: 2026-09-05 15:0x → 16:xx
**產出**: `report3090.pdf`（16 頁）、`code/make_report3090.py`、`Makefile.report`、
`notebooks/figures/m5_{roc_pr,confusion,training,importance,timeline}.pdf`

### 補上的訓練評估（使用者指出缺這一塊）
`predictor_metrics.csv` 新增 `average_precision` 與四格混淆矩陣的**原始計數**
（`cm_drop_correct` / `cm_drop_wrong` / `cm_keep_correct` / `cm_keep_wrong`）＋精確率／召回率；
每個 run 另存 `curves_*.npz`（ROC 與 PR 的點）與 `evals_*.json`（逐迭代的訓練／驗證損失）。
四張新圖：ROC+PR、混淆矩陣、訓練曲線、特徵重要度。

**混淆矩陣直接顯示門檻的作用**（toolagent、W=35×）：
門檻 0.5 時「丟錯」115,826 次（付重算）、「留錯」23,930 次；
移到 $p^{*}$ 之後「丟錯」剩 1,290、「留錯」升到 425,204。
\key{拿大量便宜的錯誤換掉少量昂貴的錯誤}，總成本 667,207 → 132,291 ms（−80%）。

### 🔴 精度階實作進模擬器：它是過渡狀態，不是常駐狀態
使用者指出時序圖看不到 GPU 的升降級。把 `Gpu-INT8` 加進動作空間
（只放 INT8，因為 §7 的量測顯示 FP8 與 INT4 過不了品質閘），
三個常數皆實測：容量倍數 **1.94**、反量化 **0.0124 ms/block**
（**用 INT4 的實測值當上界**，因反量化成本隨位元數減少而上升）、品質由 ε 量測背書。

實測（長上下文 131K、120 請求）：**降級 780,108 次，總時間變化 +0.004%**
（1,620,189 vs 1,620,128 ms）。追蹤的 48 個 block 在 120 個請求邊界上**一次都沒有停在 INT8**。

**原因**：逐出順序沒變，降級後它仍是最冷的一個，下一次需要空間就被搬走。
要讓 INT8 成為常駐狀態，必須讓「降級之後更不該被逐出」——即改用密度排序
（逐出 $\hat\tau \times$ 位元組最大者，LHD 的想法）。
**實測那個改動把同一設定的 +15.40% 變成 −18.00%。**
兩者都量過：**精度階要能常駐，代價是換掉整個逐出準則**。

### 過程中的兩個實作錯誤（都被自己的量測抓到）
1. **第一版的降級是 no-op**：降級後把 block 用**原鍵**推回 heap，下一輪立刻又被選為最冷而逐出，
   等於白降一次——兩個版本的總時間**逐位元相同**，這就是被抓到的方式。
   修法：降級過的 block 這一輪先扣在手上，最後才放回 heap。
2. **`--tier-order` 曾經完全沒接上**：參數收了但 `tkey` 的替換沒套用，
   三個設定的輸出與未開啟時**完全相同**（−458.12%／−171.37%／+15.40%），
   一度被我讀成「這個軸沒有效果」。接上之後才量到真正的效果（−96%／−29%／−0.13%）。
   **「兩個設定的數字完全一樣」永遠要先懷疑參數沒接上。**

### 正式報告
`code/make_report3090.py` 產生 `report3090.tex`：**19 張表全部從 CSV 產生，沒有硬編數值**，
讀不到來源就中止。編譯 0 字型警告／0 未定義引用／1 個 overfull box。
內容涵蓋 M1–M5 全部量測、七狀態動作空間、完整 baseline 表（兩個模型 × 四個 ctx × 五個系統）、
Oracle 判定與本輪修正、品質 ε、預測器的訓練與評估、線上策略與失效模式、八個已修正的錯誤。
