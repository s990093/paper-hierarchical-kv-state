...
本節的組織依證據強度而非依實驗編號：\S\ref{sec:platform-setup}--\S\ref{sec:baseline-design} 給出設定，\S\ref{sec:measured}--\S\ref{sec:awq-unlock} 給出七項量測結果，\S\ref{sec:verdict} 依 \S\ref{sec:eval} 開篇訂定的判準給出 go/no-go 判定。
...
\textbf{我們不宣稱磁碟階於 NVMe 上不可部署。}該判定需要 NVMe 的持續寫入量測，而 \S\ref{app:cost} 記錄我們沒有做。可宣稱者為兩點：寫入頻寬是磁碟階一道獨立於延遲的可行性約束，而表 \ref{tab:baselines} 的五個 baseline 無一將其納入設計；成本感知放置把這道約束放寬 4--7 倍。
...
\paragraph{oracle 如何計算。}
oracle 取得整條 trace 的完整未來存取序列，據此構造一個離線放置策略。GPU 階的逐出用 Bélády/MIN（逐出下次使用最遠者），單階時此規則可證明最優~\cite{belady1966}。被逐出的 block 需再選一個目的地，我們跑兩條規則並取較佳者：\emph{cascade} 無條件往下推（CPU 有位子就放 CPU、否則放 SSD），\emph{cost-aware} 只在該階成本低於丟棄的邊際成本時才放。兩條都是可實作的離線策略，取其較佳者仍然可實作。Fang 等人~\cite{hetmem2025} 已為兩層（HBM 與 off-package DRAM）的無損放置推導上界；我們的 oracle 在兩點上延伸之：納入 SSD 層，以及納入 \act{Drop}$\rightarrow$重算動作。

三項細節決定數字的正確性。其一，重算按 block 的\emph{絕對位置}計價（式 \eqref{eq:recompute-position}）；若讓 oracle 一律以位置 0 的價格重算，它付的價會低於線上策略，等於 oracle 自己作弊。其二，前綴語意下丟棄一個 block 的邊際成本是它到請求結尾的\emph{整條尾巴}而非它自己那一塊，因為第一個缺口之後的 block 一律重算。其三，逐出去向逐項計數：若「不再用到、丟棄成本為零」的逐出佔滿全部，則 CPU 與 SSD 階對最優解毫無貢獻，量到的 headroom 全部來自 GPU 階內的逐出選擇，不足以支持多階動作空間。

\paragraph{此 oracle 給的是下界，不是上界。}
多階且動作成本異質的最佳放置我們並未求解。貪婪的目的地選擇不保證全域最優，且 oracle 的逐出\emph{未}做前綴感知——真正的最優解會刻意讓保留的 block 構成連續前綴以避免製造缺口，而本 oracle 一樣會被缺口懲罰。兩項都使其偏弱，故所報 headroom 為\textbf{下界}，真正的最優只會更高。此不對稱使 GO 判定安全，NO-GO 判定則須額外審視。與 Sibyl~\cite{sibyl2022} 報告「達到全知 oracle 的 80\%」的評估傳統一致，我們以此上界作為判準而非成績。

\paragraph{oracle 的動作集合是四個無損動作，不是六階。}
本節的 oracle 只在 \act{Gpu}$_{16}$、\act{Cpu}、\act{Ssd}、\act{Drop} 上求解，\textbf{未納入三個精度階}。這四個動作於 \S\ref{sec:eps-task} 已驗證為逐字元無損，故 $\epsilon$ 約束在此自動滿足，表 \ref{tab:verdict} 的 headroom 是\emph{位置分層本身}在 $\epsilon = 0$ 下可得的空間。精度階能額外帶來多少空間，本文未量。
