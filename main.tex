...
\paragraph{本稿狀態。}
本文為初稿。第 \ref{sec:motivation} 節的量化分析與第 \ref{sec:related} 節的文獻比對均已完成並經查證。第 \ref{sec:eval} 節的每一個數字均可追溯至一條指令與一個輸出檔，凡未量到者標為 \texttt{NOT\_MEASURED}；其中四項量測推翻了本文原先的前提，我們保留被推翻的敘述並逐項列出後果，而非改寫成事後看來正確的樣子。\textbf{第 \ref{sec:design} 節的線上策略尚未實作}，故本文不報告任何線上策略的效能；已設計但未執行的實驗列於附錄 \ref{app:notyet}。我們在第 \ref{sec:limitations} 節說明此一狀態的意涵。
...
「兩者同數量級」$\rightarrow$ 成立，且\emph{次序隨 block 的絕對位置反轉}，交叉點 $P^{*}$ 落在 10{,}851（\texttt{llama-bf16}）至 37{,}717\,token（\texttt{qwen-awq}），二者皆於 NVMe&
\act{Drop} 的成本隨位置線性成長而 \act{SSD} 為常數，故位置 $<P^{*}$ 時 \act{Drop} 較廉、其後 \act{SSD} 較廉。於 128K 上下文，落在 \act{Drop} 一側的 block 佔 8.3\%（\texttt{llama-bf16}）或 28.8\%（\texttt{qwen-awq}）。\textbf{動作空間的有效子集隨上下文長度\emph{與成本剖面}而變}——此即本文主張的一個實例 \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{重算成本不是常數。}
$\mathrm{recomp}(\ell)$ 隨 block 的絕對位置 $\ell$ 成長。\textbf{此處原先寫「因 attention 的二次項」，已由 \S\ref{sec:measured} 的量測推翻：形式為線性。}我們將其量測為位置的函數而非單一純量，並以線性層的 $2N_{\text{active}}$ 項作為 $\ell \to 0$ 的下界檢核。對 MoE 模型，$N_{\text{active}}$ 為啟用專家的參數量而非總參數量。
