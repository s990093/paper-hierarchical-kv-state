...
壓力來源 & \multicolumn{2}{c}{單請求上下文（同一條軸）} \\
主要壓力軸 & context 8K$\rightarrow$262K & context 128K$\rightarrow$1M \\
可達區間 & 8B 模型 & 更大模型 $+$ 1M \\
生態 & CUDA（baseline 齊全） & ROCm（部分受限） \\
能耗量測 & 整卡 & \textbf{HBM 分軌} \\
角色 & 機制驗證 + 完整對照 & 軸的遠端 + $\kappa$ 的第二點 \\
\bottomrule
\end{tabular}
\end{table}
...
消費級顯卡缺乏記憶體與計算分軌的能耗計數器，能耗分析僅於平台 B 進行。
