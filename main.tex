...
我們固定記憶體壓力為 $5\times$、重用率 80.9\%，僅改變請求長度（GPU 預算隨工作集等比放大）。平台 A 上 \texttt{qwen-awq} 剖面的 $P^{*} = 37{,}717$\,token（表 \ref{tab:costmodels}），量測結果見表 \ref{tab:sweet-spot}：headroom 於 32K 時僅 8.94\%，於 65K--131K 升至 31.75\%--33.50\%，於 262K 降至 24.44\%，於 524K 進一步降至 12.71\%。\textbf{峰值落在 $3.5 P^{*}$ 處。}

\begin{table}[t]
\centering\small
\caption{固定壓力（$5\times$）與重用率（80.9\%）下，headroom 隨請求長度的變化。平台 A、\texttt{qwen-awq} 剖面、NVMe 磁碟階、僅計 prefill。$P^{*} = 37{,}717$（表 \ref{tab:costmodels}）。}
\label{tab:sweet-spot}
\begin{tabular}{rrl}
\toprule
請求長度 & headroom & 判定 \\
\midrule
32{,}768 & 8.94\% & MARGINAL \\
65{,}536 & \textbf{31.75\%} & GO \\
131{,}072 & \textbf{33.50\%} & GO \\
262{,}144 & 24.44\% & GO \\
524{,}288 & 12.71\% & MARGINAL$^\dagger$ \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize $^\dagger$ 該設定下單一請求的 KV 超出 GPU 預算，且該長度的上半段（位置 258{,}048 以上）超出 \texttt{qwen-awq} 成本模型的擬合上限，為線性外插。}
\end{table}
