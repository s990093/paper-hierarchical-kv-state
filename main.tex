...
式 \ref{eq:crossover} 的分母 $\alpha$ 由 prefill 吞吐決定。加速器越快則 $\alpha$ 越小、$P^{*}$ 越大、峰值長度越長。以實測的 $\alpha$ 按倍率縮放模擬，峰值自平台 A 的 256K 移至 6 倍算力下的 512K，且峰值 headroom 自 18.7\% 升至 39.5\%。\textbf{此為可證偽的預測}：$\alpha$ 的縮放為推算而非量測，於 A100 或 H100 上重量 \S\ref{app:cost} 的成本常數即可驗證。其政策意涵為\emph{同一套放置策略不可能同時適用於不同加速器}，即本文第 \ref{sec:motivation} 節主張的量化形式。
