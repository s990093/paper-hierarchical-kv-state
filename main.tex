...
式 \ref{eq:crossover} 的分母 $\alpha$ 由 prefill 吞吐決定。加速器越快則 $\alpha$ 越小、$P^{*}$ 越大、峰值長度越長。以實測的 $\alpha$ 按倍率縮放模擬（表 \ref{tab:hw-peak}），峰值自平台 A 的 256K 移至 6 倍算力下的 512K，且峰值 headroom 自 18.7\% 升至 39.5\%。其政策意涵為\emph{同一套放置策略不可能同時適用於不同加速器}，即本文第 \ref{sec:motivation} 節主張的量化形式。

\begin{table}[t]
\centering\small
\caption{headroom 隨請求長度與加速器算力的變化（壓力 $5\times$、重用率 80.9\%、僅計 prefill）。\textbf{粗體為每一列的峰值。}算力倍率係將平台 A 實測的 $\alpha$ 按比例縮放，為推算而非量測。}
\label{tab:hw-peak}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
加速器 & $P^{*}$ & 64K & 128K & 256K & 512K & 768K \\
\midrule
平台 A（實測） & 37{,}7\,K & 0.0 & 17.5 & \textbf{18.7} & 15.5 & 8.7 \\
$2\times$ 算力 & 75{,}4\,K & 2.7 & 23.8 & \textbf{28.3} & 25.2 & 16.4 \\
$4\times$ 算力 & 150{,}9\,K & 5.1 & 20.6 & \textbf{37.5} & 34.7 & 25.0 \\
$6\times$ 算力 & 226{,}3\,K & 5.8 & 18.9 & 34.7 & \textbf{39.5} & 29.7 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{於 MI300X，單請求長度\emph{不可能}到達峰值。}
以表 \ref{tab:ratio} 的 BF16 峰值算力比（$1{,}307.4 / 71.2 = 18.4\times$）外推，平台 B 的 $P^{*}$ 為 346K--693K（視 MFU 而定），對應的峰值長度為 1.2M--2.4M\,token。\textbf{此長度遠超出生產工作負載中的單請求長度}：Mooncake 的最長請求為 126{,}208\,token，僅為該峰值的 1/10 至 1/19。

其推論是一項\textbf{可直接驗證的預測}：於 MI300X，$2$--$3.5\,P^{*}$ 的甜蜜點落在 692K--2.4M\,token，故 \textbf{1M 級的單請求上下文恰落於該區間之內}。Llama-3.1-8B 於 1M 上下文需 122\,GB KV，單張 MI300X 可容納（表 \ref{tab:fit}），因此這個預測在單卡上就能證實或推翻——這正是平台 B 的實驗設定。若於 1M 量到的 headroom 顯著低於平台 A 峰值的量級，本節的機制解釋即被推翻。
