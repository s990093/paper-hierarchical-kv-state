...
\node[font=\scriptsize\bfseries\sffamily,text=dropcol!55!black,anchor=north]
  at ($(\xB,\yf)+(0,-0.42)$) {平台 A 實測：SSD 取回不快於重算 $\Rightarrow$ 重算是階梯上的一層，而非失敗路徑};
...
\addlinespace
\act{SSD} 與 \act{Drop} 的支配關係 &
「兩者同數量級，應共同決策」$\rightarrow$ 同數量級成立，但 \act{SSD}（346\,$\mu$s/token）\emph{劣於} \act{Drop}（326\,$\mu$s/token） &
\act{Drop} 在平時成本與被需要時成本兩軸皆不劣於 \act{SSD}，故 \act{SSD} 於本平台為被支配的動作。\textbf{最佳動作空間本身隨硬體而變}——此即本文主張的一個實例 \\
\bottomrule
\end{tabular}
\end{table*}
...
第四處修正的性質不同。\act{SSD} 在平台 A 的取回成本（346\,$\mu$s/token）高於 \act{Drop}（326\,$\mu$s/token），而 \act{Drop} 的平時成本為零，故 \act{SSD} 在兩軸上皆被支配。這不是本文主張的反例，而是其直接實例：\emph{最佳動作空間本身隨硬體而變}。六階動作空間應理解為候選集合，其有效子集由平台決定。
...
我們於 SATA SSD（Samsung 870 QVO）與 NVMe（Crucial P3）各量一次。裝置頻寬分別為 0.49 與 1.65\,GB/s（相差 $3.4\times$），而端到端 warm TTFT 為 5{,}803 與 6{,}600\,ms——\emph{頻寬較高者反而較慢}。磁碟讀取僅佔搬移成本的約 35\%，其餘為分層機制的查表、cascade 排程與 promotion 開銷。故成本模型應拆為 $C_{\text{ssd}}=C_{\text{lookup}}+C_{\text{sched}}+C_{\text{io}}(\text{device})$，僅末項與裝置有關。
...
模擬器的效力取決於兩個前提。其一，成本常數須為實測；程式在讀不到 \S\ref{app:cost} 的量測檔時中止，不使用預設值。其二，模擬器須能複現已量測的行為；我們以第 \ref{sec:eval} 節的工作負載比對 \texttt{full\_gpu} 相對 \texttt{cpu\_lru} 的 warm 段成本比，方向一致（2/2），但量級高估約 $1.5\times$（實測 9.00 對模擬 14.33）。\textbf{故本文僅引用 headroom 隨工作負載偏斜的趨勢，不引用其絕對值。}

已知的偏差來源為前綴語意：vLLM 的 prefix cache 於中間缺塊時須重算其後全部，而模擬器記為重算單一 block，因而低估 \texttt{full\_gpu} 的成本。修正此項後 headroom 只會上升。
