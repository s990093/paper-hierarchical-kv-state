...
此式擬合自 \texttt{llama-bf16} 剖面的五個位置（$P\le24{,}576$），最大偏差 1.6\%，五點皆落於直線上，\emph{未觀察到二次成分}；自 $P=0$ 至 $P=24{,}576$ 成本成長 2.29 倍。\texttt{qwen-awq} 剖面另有十一個位置的量測（$P\le258{,}048$），線性形式在放大 $10.5\times$ 的位置範圍上仍成立，成本自 453.9 成長至 6{,}320.2\,ms（$13.9\times$）。係數依平台與模型而定（表 \ref{tab:costmodels}），但\emph{線性}這個形式來自 $O(C\cdot P)$，與平台無關。將重算視為單一常數會在長上下文下系統性低估其成本。
...
\textbf{模型剖面}。主設定為 Qwen2.5-7B-Instruct-1M（AWQ-INT4 權重，記為 \texttt{qwen-awq}），對照為 Llama-3.1-8B-Instruct 的 BF16 與 AWQ 權重（\texttt{llama-bf16}、\texttt{llama-awq}）。兩個剖面的 $\kappa$ 相差 2 倍，提供\emph{同一硬體上}的第二個資料點。成本常數隨剖面變動，三組並列於表 \ref{tab:costmodels}；本節每一處引用皆標明用的是哪一組。
...
於真實 trace 上，\texttt{qwen-awq} 剖面的端到端 headroom 為 8.42\%（toolagent）與 9.23\%（conversation），落於 \S\ref{sec:eval} 判準的 5--15\% 區間。\textbf{但 Mooncake 的請求長度中位數僅 6{,}352\,token}，遠低於本文目標區間；於 65K--262K 的長度上端到端 headroom 為 17\%--29\%（\S\ref{sec:sweet-spot}）。
...
數值分析（\texttt{code/quant\_error.py}）顯示：FP8 靜態縮放的相對誤差為 2.64\%，改用 per-token-head 動態縮放僅降至 2.56\%，而 INT8 動態縮放為 0.65\%。FP8 e4m3 的 3 個尾數位元將相對精度鎖定於約 12.5\%，縮放方式無法改變；INT8 於指定範圍內有 255 個刻度。理論值（FP8 $12.5\%/\sqrt{12}=3.6\%$、INT8 $3/440=0.68\%$）與實測吻合。數值誤差並可單調預測檢索結果：0.65\%$\to$95\%、2.64\%$\to$5\%、11.76\%$\to$0\%。
...
常數的來源檔記於 \texttt{results/m2\_harness/retrieval\_cost\_*.csv} 與 \texttt{recompute\_position*.csv}。\texttt{qwen-awq} 的取回量測只在 NVMe 上進行，故該剖面沒有 SATA 的對應值。
...
\paragraph{不變量檢查。}
\texttt{code/m4\_invariants.py} 在每次掃描前後自動執行。開跑前檢驗 trace 的單位、成本模型的外插範圍、以及磁碟階容量是否物理可行；跑完後檢驗三項：命中數守恆（各階命中與重算之和等於總存取數）、強制未命中下限（重算次數不得低於不重複 block 數）、以及 oracle 為上界（oracle 的總成本不得高於任一線上策略）。任一項失敗即代表模擬器有錯，結果不得採用。
...
\paragraph{效力前提。}
其一，成本常數須為實測；程式在讀不到 \S\ref{app:cost} 的量測檔時中止，且模型剖面（GPU 預算、KV 每 token 位元組、成本常數的來源模型）三者必須一致，否則拒絕執行。其二，模擬器須能複現已量測的行為。我們以第 \ref{sec:eval} 節的工作負載比對 \texttt{full\_gpu} 相對 \texttt{cpu\_lru} 的 warm 段成本比：16K 上下文實測 9.00 對模擬 9.73（差 8\%），32K 上下文實測 12.27 對模擬 12.66（差 3\%）。絕對值同樣相符，\texttt{full\_gpu} 為 5{,}033 對 5{,}864\,ms、\texttt{cpu\_lru} 為 559 對 603\,ms。\textbf{驗證僅涵蓋一種工作負載形狀與兩個有鑑別力的上下文長度，本文引用 headroom 的絕對值時一併標明此範圍。}
