...
\textbf{這張卡不是主張的一部分。}本文的主張以\emph{成本常數}表述——$\kappa$、$P^{*}$、$\alpha$——而非以特定加速器表述。3090 是目前可取得、能把這些常數量到的機器；換一張卡，常數改變而形式不變，所有判別式照樣算得出來（\S\ref{sec:sweet-spot} 以縮放後的 $\alpha$ 給出四個算力級距的預測）。本節凡出現「平台 A」處，讀者應理解為「常數取自表 \ref{tab:costmodels} 的這一組」，而非「此結論僅適用於 3090」。反過來說，凡是真正只在 sm\_86 成立的結果（如 \S\ref{sec:eps-task} 的 FP8 後端限制），我們明確標為硬體相依。
...
\paragraph{以上三段皆為\emph{可證偽的預測}，而非量測。}
...
\paragraph{此判定不宣稱什麼。}
我們\emph{不}宣稱一個適用於一般情形的 headroom 數字：表 \ref{tab:verdict} 的五列相差 $12\times$，取其一為代表皆為誤導。我們\emph{不}宣稱線上策略能取得 oracle 的任何比例，該量測需策略實作，尚未進行。我們\emph{不}宣稱平台 B 的任何數字。末列的 65K--131K 為合成長上下文，其重用率與長度分布由我們設定而非取自生產環境；\textbf{目前沒有公開 trace 同時具備長上下文與真實的低重用率}，這是本文最實質的證據缺口（\S\ref{sec:limitations}）。
