...
我們設計了雙平台實驗（RTX 3090 的 30:1 與 MI300X 的 84:1，相距 2.8 倍），使硬體條件化的主張成為可證偽的。二者\emph{並非}此光譜的兩端——GH200 的 9:1 更低，MI355X 的 127:1 更高——但構成我們實際可取得的最大跨度。\textbf{本文為初稿。}第 \ref{sec:eval} 節給出完整的實驗計畫與消融設計；第 \ref{sec:measured-results} 節列出已完成的量測，其中數項推翻了本文原先的前提。主結果表格尚未取得。
\end{abstract}
...
\paragraph{本稿狀態。}
本文為初稿。第 \ref{sec:motivation} 節的量化分析與第 \ref{sec:related} 節的文獻比對均已完成並經查證；第 \ref{sec:eval} 節為實驗計畫，尚未執行完畢；第 \ref{sec:measured-results} 節列出已完成的量測，每個數字均可追溯至一條指令與一個輸出檔。我們在第 \ref{sec:limitations} 節誠實說明此一狀態的意涵。
...
\textbf{本節為實驗\emph{計畫}：它說明要量什麼、怎麼量、以及什麼結果會推翻本文的主張。本節的結果表格均為佔位符。}已經完成的量測另列於第 \ref{sec:measured-results} 節，兩者分開陳述是因為證據地位不同。
...
本節僅列出\emph{已經量到}的結果。第 \ref{sec:eval} 節的計畫尚未執行完畢，其結果表格仍為佔位符。分開陳述的理由是兩者的證據地位不同：本節的每一個數字都可追溯至一條指令與一個輸出檔，而該節的表格是待填的。
...
於真實 trace 上，\texttt{qwen-awq} 剖面的端到端 headroom 為 8.46\%（toolagent）與 9.28\%（conversation），落於 \S\ref{sec:eval} 判準的 5--15\% 區間。\textbf{但 Mooncake 的請求長度中位數僅 6{,}352\,token}，遠低於本文目標區間；於 65K--262K 的長度上端到端 headroom 為 17\%--29\%（\S\ref{sec:sweet-spot}）。

\subsection{品質退化是「精度 $\times$ 任務」的性質，而非精度的性質}
\label{sec:eps-task}
...
\subsection{寫入頻寬使勝出的 baseline 不可部署}
\label{sec:write-feasibility}

模擬的成本模型僅對「取回」計價，而寫入亦消耗頻寬。以 trace 的真實時長換算，\texttt{tier\_fs} 於 toolagent 上需 4{,}666\,MiB/s 的持續磁碟寫入。實測（\S\ref{app:cost}）SATA QLC 為 190\,MiB/s、NVMe 為 2{,}512\,MiB/s，\textbf{兩者皆無法達成}；即使以「每個不重複 block 僅寫一次」的下界計算仍需 3{,}086\,MiB/s。Oracle 的成本感知放置僅需 1{,}114--1{,}214\,MiB/s，於 NVMe 上可行。

其意涵為：\texttt{tier\_fs} 雖於模擬中勝出，卻非可部署的對照組。\textbf{成本感知放置的價值因此不僅是延遲，更是使磁碟階成為可用選項的前提。}
...
我們固定記憶體壓力為 $5\times$、重用率 80.9\%，僅改變請求長度（GPU 預算隨工作集等比放大）。平台 A 上 $P^{*} = 37{,}615$\,token，量測結果見表 \ref{tab:sweet-spot}：headroom 於 32K 時僅 8.94\%，於 65K--131K 升至 31.75\%--33.50\%，於 262K 降至 24.44\%，於 524K 進一步降至 12.71\%。\textbf{峰值落在 $3.5 P^{*}$ 處。}

\begin{table}[t]
\centering\small
\caption{固定壓力（$5\times$）與重用率（80.9\%）下，headroom 隨請求長度的變化。平台 A、\texttt{qwen-awq} 剖面、NVMe 磁碟階、僅計 prefill。$P^{*} = 37{,}615$。}
...
\vspace{2pt}
{\footnotesize $^\dagger$ 該設定下單一請求的 KV 超出 GPU 預算，且 78.6\% 的存取位置超出成本模型的擬合上限（114{,}688\,token），為線性外插。}
\end{table}
...
$2\times$ 算力 & 75{,}2\,K & 2.6 & 23.8 & \textbf{28.3} & 25.2 & 16.3 \\
$4\times$ 算力 & 150{,}5\,K & 5.0 & 20.6 & \textbf{37.5} & 34.7 & 25.0 \\
$6\times$ 算力 & 225{,}7\,K & 5.8 & 18.9 & 34.7 & \textbf{39.5} & 29.7 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{於 MI300X，單請求長度\emph{不可能}到達峰值。}
以表 \ref{tab:ratio} 的 BF16 峰值算力比（$1{,}307.4 / 71.2 = 18.4\times$）外推，平台 B 的 $P^{*}$ 為 346K--693K（視 MFU 而定），對應的峰值長度為 1.2M--2.4M\,token。\textbf{此長度超出所有可取得模型的原生上下文上限}（表 \ref{tab:kvsize} 中最長者為 Llama-4-Scout 的 10.5M，但其 KV 為 192\,KiB/token；Qwen2.5-1M 為 1.01M）。
...
\paragraph{平台 A 的上下文上限為 262{,}144，且換卡無法解除。}
Qwen2.5-7B-Instruct-1M 的原生 1M 上下文依賴 Dual Chunk Attention。vLLM 0.28.0 的 V1 engine 無法執行該路徑：載入含 \texttt{dual\_chunk\_attention\_config} 的 checkpoint 時，\texttt{qwen2.py} 會向 attention 實作額外傳入 \texttt{layer\_idx}，而 \texttt{FlashInferImpl} 不接受此參數，engine 於初始化即崩潰。改用移除該設定的變體後上限為 \texttt{max\_position\_embeddings} 的 262{,}144；於 524{,}288 送出請求會觸發 \texttt{CUDA error: device-side assert triggered}（RoPE 索引越界），\emph{延遲亦無法量測}，而非僅品質無效。

此限制源於模型與框架而非記憶體：該組態的 KV 容量為 559{,}584\,token，遠超 525{,}568 的需求。\textbf{因此更大的加速器無法解除此上限}；欲於本文的目標區間取得品質數據，需更換模型或 vLLM 版本。第 \ref{sec:sweet-spot} 節於 524K 的數值因而標為外插，不作為結論依據。
