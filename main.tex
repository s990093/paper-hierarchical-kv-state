...
\subsection{四項成本量測決定了動作空間的有效子集}
\label{sec:measured}

動作空間 $\mathcal{A}$ 的六個狀態並非在每個平台上都存在，也並非在每個上下文長度下都有意義。表 \ref{tab:measured} 給出四項決定其有效子集的量測：前兩項確立 GPU 上四個精度階的\emph{可用性}與\emph{實際容量收益}，後兩項確立 \act{Drop} 的成本形式與它相對 \act{SSD} 的適用區間。量測協定與逐設定的完整數據見附錄 \ref{app:measurements}。

\begin{table*}[t]
\centering\small
\caption{決定動作空間有效子集的四項量測（平台 A，RTX 3090, sm\_86）。}
\label{tab:measured}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.29\textwidth}>{\raggedright\arraybackslash}p{0.44\textwidth}}
\toprule
\textbf{量測} & \textbf{結果} & \textbf{對動作空間與成本模型的意涵} \\
\midrule
FP8 KV 於 sm\_86 &
可用，容量恰增 $2.00\times$（41{,}648$\rightarrow$83{,}312\,token） &
sm\_86 無原生 FP8 \emph{tensor core 運算}，但 \act{Gpu}$_8$ 是\emph{儲存}狀態，與 tensor core 無關。GPU 上的四個精度階皆為 \texttt{-{}-kv-cache-dtype} 的取值，單一平台即可驗證 \\
\addlinespace
精度階梯的實際收益 &
$2.00\times$/$1.94\times$/$3.77\times$，而非位元寬度所示的 $2\times$/$4\times$ &
per-token-head 量化固定多帶 2\,KiB/token 的 scale（$2\times32\times8$ 個 FP32）。量化中繼資料與序列長度成正比，必須進入成本模型；僅以每元素位元組數計算會高估低精度階 \\
\addlinespace
$C_{\text{recompute}}$ 對位置的形式 &
對位置\emph{線性}，式 \eqref{eq:recompute-position}，擬合偏差 $<1.6\%$ &
整段 prefill 為 $O(L^2)$，但固定大小 block 在位置 $P$ 重算只需與既有前綴做 attention，為 $O(C\cdot P)$。二者是不同的量，將 \act{Drop} 視為單一常數會在長上下文下系統性低估其成本 \\
\addlinespace
\act{SSD} 與 \act{Drop} 的優劣次序 &
同數量級，且\emph{次序隨 block 的絕對位置反轉}，交叉點 $P^{*}$ 落在 10{,}851（\texttt{llama-bf16}）至 37{,}717\,token（\texttt{qwen-awq}），二者皆於 NVMe &
\act{Drop} 的成本隨位置線性成長而 \act{SSD} 為常數，故位置 $<P^{*}$ 時 \act{Drop} 較廉、其後 \act{SSD} 較廉。於 128K 上下文，落在 \act{Drop} 一側的 block 佔 8.3\%（\texttt{llama-bf16}）或 28.8\%（\texttt{qwen-awq}）。\textbf{動作空間的有效子集隨上下文長度\emph{與成本剖面}而變}，且可由成本常數事先算出 \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Takeaway.}
四項量測共同給出一個結論：\textbf{六階動作空間是候選集合，不是每個平台上都成立的必要集合。}其有效子集由兩個可事先計算的量決定——精度階由硬體與後端支援與否決定（\S\ref{sec:eps-task} 顯示於 sm\_86 只有一階保住檢索品質），位置階由 $P^{*}$ 與請求長度的關係決定。二者皆不需試跑即可算出，這正是「策略須隨硬體條件化」主張的可操作形式。
