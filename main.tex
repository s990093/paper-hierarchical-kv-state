...
問題的\emph{性質}同樣隨平台改變。在 MI300X 上，8B 至 14B 級模型於 128K 上下文的單請求 KV footprint 僅佔 HBM 的 8--10\%；此處的壓力來自多租戶下的\emph{機會成本}，而非容量本身。在 24\,GB 級的加速器上，我們的量測顯示壓力源取決於權重精度：BF16 權重僅餘 41{,}648 token 的 KV 預算，無法進入 128K 區間；改用 AWQ-INT4 權重後，同一張卡的 KV 容量升至 120{,}320--547{,}744 token，\textbf{相差 $13.2\times$}。兩者皆為真實的部署情境，而最佳策略在二者上並不相同。
...
\paragraph{可推廣性的界線。}
我們\emph{不}由平台 A 的結果推論多租戶機會成本場景（24\,GB 無法容納多個長 session），亦\emph{不}提出能耗結論（消費級顯卡缺乏記憶體與計算分軌的能耗計數器）。
...
其推論是：於 MI300X，記憶體壓力\emph{不可能}來自單一請求的長度，而必須來自並行請求數。這與第 \ref{sec:motivation} 節的觀察一致：該節指出 MI300X 上的壓力源是多租戶下的機會成本而非容量本身。\textbf{本節的成本模型與該節的容量分析由兩條獨立的推理得到相同結論}，構成一項內部一致性檢查。

平台 B 的實驗設計因此與平台 A 不同：平台 A 掃描單請求長度（本節），平台 B 必須掃描並行度。第 \ref{sec:eval} 節的壓力軸設計已反映此差異。
