...
### B9. 精度決策的**粒度**是一條沒被檢驗的自由軸 ← 2026-09-02 新增，已寫入論文附錄 B.2 與表 15 (E)

| | |
|---|---|
| **現況** | 論文預設精度是 **per-block（16 token）**決定的——Fig. 3(b) 就是這樣畫的，動作 $a_i$ 的作用範圍是「16 token 的整欄」。但這是**未經檢驗的預設值，不是量測結論**。§6.6 的四個精度階全部是用 `--kv-cache-dtype`（**引擎級全域旗標**）量的，所有 block 同一格式；§6.2 的 oracle 又**未納入精度階**。**論文從未跑過混合精度。** |
| **新觀察** | 粒度是一條獨立的自由軸，而且有一個物理上的特徵尺度：vLLM 的 block 是 **16 token**，而 attention kernel 的 KV tile 是 **64–128 token**（FlashAttention 類），兩者差 4–8 倍。**粒度細於 tile 時，格式邊界必然掉進 tile 內部**，同一批載入的資料要用兩套規則解讀 |
| **為何是變體不是修正** | 兩端都有代價，中間才有最佳點：<br>**細粒度** → 保住「燙的 block 留 BF16、冷的壓 INT4」的自由度，但**損失 kernel 效率**<br>**粗粒度** → kernel 永遠規整，但**損失容量收益**（燙冷混在同一段，只能取折衷精度）<br>**故這是一條取捨曲線，不是一個答案。** |
| **與核心主張的關係** | 最佳粒度由 **tile 大小**決定，而 tile 大小隨 attention backend 與硬體而變 → **「常數改變則最佳策略隨之改變」的第三個實例**（前兩個：加速器世代、模型剖面）。與 $\kappa$ 軸正交，**強化而非增加主張** |
| **論文怎麼寫** | 附錄 B.2 新增 \paragraph{最佳的決策粒度由 attention tile 的大小決定，故隨後端而變。}，表 15 新增 (E) 區塊四列；caption 補 $^{\ddagger}$ 說明新增量法。編譯確認 0 overfull／0 undefined／未增頁 |
| **提議的消融 (E) 區塊** | 見下表。三個軸（品質／速度／記憶體）論文都已有統一量法，可直接比 |
| **需要新增的量法** | **只有一項**：attention kernel 層級的延遲。§B.6 現有的指標清單（TTFT / TPOT / 服務層級分佈 / 重算率）**量不到 tile 破碎**，因為它被端到端延遲吸收掉了。其餘三軸沿用既有：$Q(\pi)=\min_t r_t$（式 14）、TTFT/TPOT、**有效** KiB/token（含碎片化，非理想值） |
| **🔴 為何必須在平台 B 做** | 兩個獨立理由，第二個是決定性的：<br>**(i)** ROCm 的 attention backend 與 CUDA 不同 → tile 幾何不同 → 最佳粒度可能不同。§B.6 已記載「啟用 KV connector 會使 vLLM 自候選清單移除 `ROCM_ATTN` 後端」，故平台 B 本來就要逐次記錄實際用的 backend<br>**(ii)** **平台 A 只有 2 個精度階能用**（BF16、INT8——FP8 掉到 5%、INT4 掉到 0%，且 `fp8_per_token_head` 需 Triton 而該後端在 sm_86 拒絕 FP8 KV）。**只有兩檔可選時，粒度的差異量不出來。** CDNA3 有原生 FP8，若 `fp8_per_token_head` 在 ROCm 上可用（**須先驗證，目前 `NOT_MEASURED`**），平台 B 才有 3 階以上，這個消融才有鑑別力 |
| **不處理的風險** | **中**。若不做，Fig. 3(b) 那張「四種狀態並存」的圖就一直是**示意而無實作依據**，審稿人問「混合精度的 kernel 怎麼寫」時沒有答案。B7 已指出精度階落在 `CachePolicy` 介面**之外**、需要改動 attention 路徑——**粒度決定那個改動要多深** |
| **可能的負面結果** | 若三個粒度在兩平台上差異都不顯著 → **粒度不是有意義的設計維度**，論文應把它降為實作細節一句帶過，並把 Fig. 3(b) 改成段粒度。**這是可接受的結果，照寫。** |
| **何時做** | Oracle = GO 之後、**實作精度階之前**。順序不能反——先知道粒度，才知道 attention 路徑要改多深 |

**提議的消融 (E) 區塊**（接在表 15 的 (D) 之後）：

| 變體 | 決策單位 | 格式邊界 vs tile | 檢驗什麼 |
|---|---|---|---|
| per-block（現況預設） | 16 token | **落在 tile 內部** | 最細粒度的容量收益，減掉 kernel 破碎的代價 |
| **tile 對齊** | ~128 token | **恰好重合** | 粒度與 kernel 效率的折衷點 |
| prefix 對齊 | 512 token | 重合 | Mooncake `hash_ids` 的天然粒度 |
| 整段前綴 | 整段 | 無邊界 | 等同全域統一＝**§6.6 已量到的那一列** |

**四個變體共用的量法**：$Q(\pi)=\min_t r_t(\pi)$（式 14）、attention kernel 延遲（**新增**）、
有效 KiB/token（含碎片化）、端到端 TTFT。**兩平台分開報告**，理由同 (B) 區塊。

> ⚠️ 末列不是新實驗——它就是 §6.6 已經跑過的全域統一設定，**免費的錨點**，
> 可用來檢查新 harness 是否複現得出既有數字。

證據：`main.tex` §4.1（$a_i$ 作用範圍 16 token）、§6.6（精度 × 任務）、§6.9（`--kv-cache-dtype` 為全域旗標）、Fig. 3(b) caption 自標「示意，非量測資料」；`docs/` 的對齊問題圖解
