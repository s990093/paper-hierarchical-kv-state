...
**🔴 2026-09-05 第一次實測：預測的方向是反的。** 不必等平台 B——κ 在同一台機器上
就隨 block 的絕對位置由 12 變到 60。在預測夠難的設定（W=35×、AUC 0.896）上：
門檻移動（Elkan）本身值 **−80% 成本**，而加權訓練在其上是 **+25%（更差）**，
且**隨 κ 增大而惡化**（−2.1% @ κ=12 → −428% @ κ=60）。
依本節自訂的判準，這一輪的結論是**主張應縮減至門檻校準**。
$n{=}1$ seed、單 trace、單平台，但高 κ 區差 4 倍以上。
證據與可能的機制見 `PAPER_DELTAS.md` B6、`results/m5_predictor/cost_by_position.csv`。
